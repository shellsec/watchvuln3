...
func NewApp(config *WatchVulnAppConfig) (*WatchVulnApp, error) {
	...
	grabs, err := grab.BuildGrabbers(config.Sources)
	if err != nil {
		return nil, err
	}
		...
		if w.config.NoFilter || v.Creator.IsValuable(v) {
			...
			if v.CVE != "" && *w.config.EnableCVEFilter {
				...
				if len(others) != 0 {
					...
					w.log.Infof("found new cve but other source has already pushed, mark skipped, others: %v", ids)
					if _, err = dbVuln.Update().SetPushed(true).Save(ctx); err != nil {
						w.log.Errorf("failed to mark %s as pushed (cve dedup), %s", v.UniqueKey, err)
					}
					continue
				}
			}
