...
- 版本号升级为 **v3.0.0**，聚焦稳定性与配置一致性

### 修复

- 修复奇安信 TI 数据源 Referer/Origin 请求头错误
- 修复 `GO_SKIP_TLS_CHECK` 环境变量在 Docker/CLI 下不生效的问题
- 修复 CVE 去重跳过后每个周期重复尝试推送的问题
- 修复初始化数据源时的并发数据竞争

### 新增

- **漏洞情报看板**：`--web-addr` / `WEB_ADDR` / 配置项 `web_addr`，或子命令 `watchvuln board`（无登录）
- CLI：`watchvuln list-sources`、`watchvuln init-config`
- CLI：`--pusher-file` / `PUSHER_FILE` 独立推送列表（支持多个同类型钉钉等）
- 启动时输出配置自检摘要（数据源、推送通道、看板地址等）

### 改进

- 统一默认 `sources` 列表与文档说明
- 布尔环境变量支持 `true`/`false`/`1`/`0` 等标准解析
- 为 `--no-start-message` 增加 `--quiet` 别名，并补充关闭启动推送的文档
- 配置文件方式已支持多钉钉；文档与 `pushers.example.yaml` 示例完善
