...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		&cli.StringFlag{
			Name:     "sources",
			Aliases:  []string{"s"},
			Usage:    "set vuln sources (nox/ti are aliases for 奇安信)",
			Value:    grab.DefaultSources,
			Category: "[Launch Options]",
		},
		&cli.StringFlag{
			Name:     "interval",
		...
		&cli.BoolFlag{
			...
			Aliases:  []string{"nm", "quiet"},
			Usage:    "disable init finished push message when server starts (same as NO_START_MESSAGE=true)",
			Category: "[Launch Options]",
		},
		&cli.BoolFlag{
			Name:     "no-filter",
		...
		&cli.StringFlag{
			Name:     "web-addr",
			Usage:    "vuln intelligence board listen address, e.g. 127.0.0.1:8765 (empty to disable)",
			Category: "[Launch Options]",
		},
		&cli.StringFlag{
			Name:     "pusher-file",
	...
	app.Commands = []*cli.Command{
		{
			Name:   "list-sources",
			Usage:  "list built-in vulnerability sources",
			Action: listSourcesAction,
		},
		{
			Name:  "init-config",
			Usage: "write a commented config.yaml template",
			Flags: []cli.Flag{
		...
		{
			Name:  "board",
			Usage: "run vuln intelligence web board only (no push)",
			Flags: []cli.Flag{
				&cli.StringFlag{Name: "db-conn", Aliases: []string{"db"}, Value: "sqlite3://vuln_v3.sqlite3"},
				&cli.StringFlag{Name: "web-addr", Value: "127.0.0.1:8765", Usage: "listen address"},
			},
			Action: boardAction,
		},
	}
...
func initConfigFromFile(c *cli.Context) (*ctrl.WatchVulnAppConfig, error) {
	...
	default:
		return nil, fmt.Errorf("unsupported config file %s, use .yaml/.yml/.json", configFile)
	}
	if err != nil {
		return nil, err
	}
...
func initConfigFromCli(c *cli.Context) (*ctrl.WatchVulnAppConfig, error) {
	...
	if v, ok := util.EnvBool("NO_FILTER"); ok {
		noFilter = v
	}
	if v, ok := util.EnvBool("NO_START_MESSAGE"); ok {
		noStartMessage = v
	}
	if v, ok := util.EnvBool("NO_GITHUB_SEARCH"); ok {
		noGithubSearch = v
	}
	if v, ok := util.EnvBool("NO_SLEEP"); ok {
		noSleep = v
	}
	if v, ok := util.EnvBool("ENABLE_CVE_FILTER"); ok {
		cveFilter = v
	}
	if v, ok := util.EnvBool("DIFF"); ok {
		diff = v
	}
	if v, ok := util.EnvBool("GO_SKIP_TLS_CHECK"); ok {
		insecure = insecure || v
	}
