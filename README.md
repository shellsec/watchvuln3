...
1. **先复制**预填好的分析提示词到剪贴板（优先同步复制，失败时尝试 Clipboard API）
2. 复制成功时按钮短暂显示 **「已复制」**（约 2 秒），并打开对应 AI 站点
3. 在 AI 对话框中 **Ctrl+V** 粘贴即可

| 按钮 | 打开站点 | 说明 |
|------|----------|------|
| ChatGPT | [chatgpt.com](https://chatgpt.com/) | 提示词较短时同时通过 URL 带问题；较长时以剪贴板为准 |
| Gemini | [gemini.google.com](https://gemini.google.com/app) | 主要靠剪贴板粘贴 |
| DS | [chat.deepseek.com](https://chat.deepseek.com/) | 主要靠剪贴板粘贴 |

若自动复制失败，会弹出对话框展示完整提示词，可手动全选复制后再点「打开站点」。

> 建议用 `http://127.0.0.1:8765/` 访问看板；部分浏览器在局域网 IP（`http://192.168.x.x`）下可能限制剪贴板，可在地址栏锁图标中检查站点权限。
